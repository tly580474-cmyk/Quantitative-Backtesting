...
export function runBacktest(
  ...
  // Queue of pending orders to execute at next day's open
  let pendingOrder: Order | null = null;

  const warmupBars = strategy.warmupBars(strategyParams);
  ...
  for (let i = 0; i < totalBars; i++) {
    ...
    if (pendingOrder) {
      ...
      if (result.trade.quantity > 0) {
        ...
        orders.push(orderWithTime);
        // Still add the rejected trade for traceability
        trades.push(result.trade);
      }
    ...
    // Pre-compute indicators needed for this bar
    ...
    if (signal.action !== 'hold' && i < totalBars - 1) {
      // Only create orders if there's a next trading day
        ...
        if (portfolio.positionQuantity === 0) {
          ...
          if (estimatedQty >= config.lotSize) {
            pendingOrder = {
              ...
              executeTime: '', // Will be set when executed
              side: 'buy',
              orderType: 'market',
          ...
          // Already holding, ignore buy signal
          orders.push({
            id: crypto.randomUUID(),
            signalTime: signal.time,
            executeTime: nextCandle.time,
      ...
      // Last candle signal is cancelled
    ...
    if (i === totalBars - 1 && config.forceCloseAtEnd && portfolio.positionQuantity > 0) {
      // Execute pending order first if any
      if (pendingOrder) {
        // Force close overrides pending order
        pendingOrder = null;
      }
  ...
  // Compute actual benchmark return (buy and hold)
  if (candles.length > 0) {
