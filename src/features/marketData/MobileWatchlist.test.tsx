...
beforeEach(() => {
  ...
  request.mockImplementation(async (path: string) => {
    ...
    const item = stocks.find(stock => path === `/api/market-data/stocks/${stock.code}/quote`);
...
describe('compact mobile watchlist', () => {
  it('loads only list quotes and indices, shows all six fields, and opens secondary details', async () => {
    ...
    expect(request.mock.calls.map(([path]) => path).sort()).toEqual([
      '/api/market-data/indices/quotes', '/api/market-data/stocks/000001/quote', '/api/market-data/stocks/600519/quote',
    ]);
  ...
  it('refreshes every quote without replacing the original return baseline', async () => {
    ...
    request.mockImplementation(async (path: string) => path.includes('/indices/') ? { items: [] }
