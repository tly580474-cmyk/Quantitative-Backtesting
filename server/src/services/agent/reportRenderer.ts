...
</style></head><body data-style="${resolvedStyle}"><main class="report"><header class="hero"><p class="eyebrow">WANHANG RESEARCH · STATIC REPORT</p><h1>${escapeHtml(title)}</h1><div class="meta">结构化研究报告 · 安全静态版本</div></header><article class="paper"><div class="content">${article}</div></article><footer class="footer"><span>由万行智研生成</span><span>生成时间 ${escapeHtml(generatedAt)}</span></footer></main>
