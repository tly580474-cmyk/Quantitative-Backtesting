import { assertLockedTestCoverage } from './lockedTestValidation.js';
...
export function assertCandidateTransition(
  ...
  if (to === 'tested') {
    if (!context.lockedTestMetrics) throw new Error('锁定测试完成后才能标记为 tested');
    assertLockedTestCoverage(context.lockedTestMetrics);
  }
