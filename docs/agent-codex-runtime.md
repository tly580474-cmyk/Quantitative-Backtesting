...
- 前端附件入口支持文件选择、拖拽以及 Ctrl+V 读取剪贴板文件；只有剪贴板包含文件时才拦截粘贴，普通文字粘贴不受影响。
- 侧栏删除按 `conversation_id` 删除全部轮次、审批、事件、附件和报告；浏览器先等待 5 秒并在右下角提供撤销按钮，倒计时结束后才调用删除 API。
- Provider 或工具错误均原样保留在失败回合中。系统不自动重跑任务；仅最新失败回合显示显式重试按钮，点击后才创建同一 conversation 的下一轮。
