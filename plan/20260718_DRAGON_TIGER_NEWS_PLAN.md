...
1. 沪深交易所官方龙虎榜备源已接入。东方财富主源失败或返回空数据时，自动切换上交所、深交所官方接口，并保留事件与席位明细。
2. 财联社新版 `v1/roll/get_roll_list` 签名接口已作为独立专业新闻源接入；与东方财富并行抓取，任一来源可用即可返回，落库后参与来源健康监测。
...
7. 官媒层已接入每日新闻联播文字稿镜像：按北京时间 19:00 形成分条事件，节目尚未播出时自动回退上一期，过滤站点异常占位标题，并以 `state_media` 纳入采集心跳、来源健康和跨媒体聚合。
8. 原“自媒体”视图调整为“市场观点解读”并移动到“全部”之后。解读智能体复用服务端 `OPENAI_*` 配置，仅取近 72 小时官媒、专业财经和聚合报道，完成事件去重、主题提炼、共识/分歧比较、影响路径、风险及验证清单，报告保留材料编号和原始链接。

### 尚未进入的 Phase 2 可选项

- 新华财经、人民日报等官媒源仍属于可选扩展，不影响当前 Phase 2 的官方龙虎榜与独立新闻备源闭环。
