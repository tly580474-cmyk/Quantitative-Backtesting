...
| 自动数据 | 历史回补、增量同步、复权和质量治理 | 外部行情源 | 版本化行情数据 | 第五阶段已建立基础能力 |
| 全量历史库 | CSV 批量入库、查询优化和研究快照 | 本地全量历史行情 | MySQL 事实层与 Parquet 快照 | 5.5 阶段规划 |
| 因子研究 | IC、分层、中性化和多因子合成 | 截面行情和财务数据 | 因子报告 | 第六阶段规划 |
