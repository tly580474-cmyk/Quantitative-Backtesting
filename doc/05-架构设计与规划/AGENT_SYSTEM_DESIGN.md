...
系统把模型供应商的原始流转换为面向产品的公共事件。API、数据库公共字段和前端 DOM 不保存、不传输、不渲染模型的原始思维链。报告不再由前端布尔开关强制控制；智能体根据用户明确要求和本轮产物价值给出机器可读的 `agent-report` 决策，后端剥离控制块后按需生成静态 HTML。决策缺失或非法时默认不生成，避免普通问答被扩写成长报告。
...
- 每轮最终结果必须附带 `agent-report` JSON 决策。明确要求报告、完整研究/回测/审计或多维评估通常生成；简单问答、澄清、状态查询和短操作结果不生成。控制块不进入公共事件、报告正文或前端 DOM。
...
- 报告生成策略为 `auto`：模板风格只是确实生成报告时的呈现偏好，不是生成许可。
- 智能体只输出 Markdown 正文和机器可读决策；静态 HTML 始终由服务端渲染。
