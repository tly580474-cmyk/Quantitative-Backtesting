...
> 文档状态：实施中（M4 基础流程：确定性样例已跑通，生产链路未完成）
> 更新日期：2026-08-01
...
- 独立 Python Worker、Python 与 DuckDB 的跨运行时黄金样例，以及 Worker 沙箱和
  Orchestrator 制品交换；当前基础计算实际运行时是 Node DuckDB。
- 真实研究快照、指数历史成分、财务公告日和已发布因子版本接入；当前冒烟使用固定
  的时点数据样例。
...
因此，当前状态只能称为 `M4 foundation smoke passed`，不能称为 M4 完成。下一轮应先
将固定样例替换为只读快照上的基础流程并建立 Python/DuckDB 黄金样例，再扩展生产级
交易语义。
