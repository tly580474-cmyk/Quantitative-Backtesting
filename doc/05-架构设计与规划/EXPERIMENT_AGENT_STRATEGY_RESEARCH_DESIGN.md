...
M4 v1 之外的扩展明确进入后续里程碑：中证 500/全 A 股股票池、通用多因子优化器、行业
中性与容量模型升级、Linux 容器级任意代码沙箱、分布式消息中间件和多租户鉴权。它们不
影响本节冻结范围的完成状态，也不能在产品能力清单中提前显示为可用。

M4 扩展的实施顺序、Schema 演进、时点数据口径和逐项验收条件见
[M4 多资产研究扩展任务清单](./M4_EXTENSION_BACKLOG.md)。该清单当前全部保持未完成，
不改变本节 M4 v1 的历史验收结论。
