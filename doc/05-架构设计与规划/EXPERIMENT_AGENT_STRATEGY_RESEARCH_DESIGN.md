...
本次只建立可重复运行的最小纵向链路，不将 M4 或以下未接入能力标记为完成。

已通过自动化测试和命令行冒烟验证的范围：

- 新增严格的 `MultiAssetPlan v1.0`、`RebalancePlan v1.0` 和时点特征行 Schema；
  两类计划使用规范化 JSON SHA-256，并校验来源计划、快照、决策日期、时点股票池、
  截面特征证据 hash、单标的权重和组合总权重。
- DuckDB 基础计算平面可按时点成员关系过滤样本，按周或月选择决策截面，执行稳定
  排名，并生成等权或评分权重。并列值使用 `instrumentKey` 升序作为固定次级顺序。
- 计算平面只输出候选、分数、目标权重和证据 hash；严格 Schema 拒绝现金、订单、
  持仓或最终权益等执行平面字段。
- TypeScript 最小撮合核心可按下一交易日开盘执行目标权重，先卖后买，处理整手、
  佣金、印花税、滑点、现金不足缩量及 `tradable=false` 的不可成交残留，并生成同一
  组合的订单和资金账本。
- 固定样例已验证退选成分不会出现在后续股票池、未来截面不会改写历史决策、篡改
  hash/股票池会被拒绝、现金非负且权益恒等于现金加持仓市值。可使用
  `npm run multi-asset:smoke` 重复运行；当前固定计划 hash 为
  `1ae6ea34c42015aa8d3a6b5b8c56b7f29b672720a09789fe4f3712d2ba7fca13`。

固定样例完成后仍需继续处理、不得据此计入 M4 整体验收的范围：

- 固定 Python 计算 Worker 与 Python/DuckDB 黄金样例在后续补充记录中已经跑通；
  Worker 操作系统级隔离和大规模制品交换仍未完成。
- 真实研究快照和指数历史成分在后续补充记录中已经接入 20 日动量基础链路；财务
  公告日、已发布因子版本和多因子组合仍未完成。
- 多资产计划和结果的数据库持久化、任务队列、服务端 Worker、API、进度事件和 UI。
- 完整逐日盯市、停牌日历、涨跌停、退市、复权/公司行为、容量、部分成交、换手和
  行业暴露；当前只实现调仓日开盘的最小资金账本。
- 与因子研究、冠军/挑战者治理的信号与收益全量一致性测试，以及生产规模、并发、
  故障恢复和安全测试。

固定样例本身只能称为 `M4 foundation smoke passed`，不能称为 M4 完成；随后完成的
只读快照与跨运行时进度记录如下。
...
- 增加固定实现的 Python 横截面 Worker。Worker 只接受 `MultiAssetPlan + 时点特征行`
  JSON，不执行用户代码，不包含资金、订单、持仓或权益逻辑；Node Orchestrator 在
  输入端执行严格 Schema 校验，并限制输入大小、运行时间、stdout 和 stderr。
- Python 与 DuckDB 黄金样例已覆盖周频等权和月频评分权重；两者的决策日、时点股票
  池、特征证据 hash、排名及目标权重完全一致，同一计划进入 TypeScript 后产生完全
  相同的订单和资金账本。
- 增加只读研究快照适配器，从已发布快照读取历史指数成分版本与日线行情，按每个交易
  日计算 20 日动量，并把决策日信息截点映射到下一交易日开盘。没有当日行情的标的仅
  使用最近收盘价估值并标记为不可交易。
- `npm run multi-asset:snapshot-smoke` 已在快照
  `eb260586-a8d0-4c24-9171-218b1222341c-20260731103228` 上实际通过：沪深 300、
  2026-06-01 至 2026-07-30、12,900 条时点输入、9 次调仓、Python/DuckDB 一致、
  TypeScript 生成 110 笔订单和 9 个账本节点；固定初始资金 100 万元，期末权益为
  975,090.97 元。该数值只用于链路复现，不代表策略评价或收益承诺。

至此，“只读真实快照 → DuckDB/Python 计划 → TypeScript 组合账本”的 M4 基础流程
已经跑通。以下仍属于完善性或生产化工作，继续保持未完成：

- 多资产计划、任务、制品和结果的数据库持久化、队列、API、进度事件与 UI；
- Python Worker 的操作系统级隔离、CPU/内存配额、Arrow/Parquet 大规模制品交换和
  故障恢复；
- 完整逐日权益、涨跌停、退市、公司行为、容量、部分成交、行业约束及多因子组合；
- 与因子研究已发布版本、冠军/挑战者治理和报告门禁的生产级对接与全量黄金样例。

上述项目不得据此补充记录标记为完成，M4 整体仍处于实施中。

#### M4 持久化与 API 第一阶段补充记录

只读快照基础流程通过后，已接入第一阶段服务端管理闭环：

- 迁移 `0038_multi_asset_runs.sql` 新增不可变计划版本和运行记录。计划保存快照绑定、
  规范化计划及计划 hash；运行保存输入 hash、幂等键、状态、进度、`RebalancePlan`、
  订单/资金账本和结果 hash。迁移已在当前开发数据库成功应用且无错误。
- 冻结计划会先验证真实快照和时点数据可用性；同一计划 hash 并发或重复提交时复用
  已有版本。运行使用全局唯一幂等键，同键同输入复用，同键不同输入返回冲突。
- 运行状态采用 `queued → running → completed | failed`。只有 `queued` 能通过条件更新
  抢占为 `running`，只有 `running` 能更新进度和写入最终结果，避免重复 Worker 同时
  覆盖同一运行。
- 新增计划创建/列表/详情、运行启动/列表/详情 API。首次启动返回 `202`，重放已存在
  的同输入运行返回 `200`，幂等冲突返回 `409`；数据库离线时统一返回 `503`。
- 真实持久化冒烟已完成：计划 `1c5cf3c5-dcbc-460e-aeb4-b158f8eadabf`、运行
  `47b8ca4b-b99e-46da-b553-b31063631722` 成功写入 `completed`，结果 hash 为
  `85982e15650be73773b4e60bc2c8b929c1b617e52f78f81e5c35ee4383f5a324`；同键重放复用，
  修改初始资金的同键请求被拒绝。
- Fastify API 冒烟同样通过：异步首次启动 `202`、完成查询成功、同输入重放 `200`、
  冲突重放 `409`。专项测试现覆盖协议、跨运行时一致性、资金守恒、配置语义、稳定
  输入 hash 和数据库离线 API。

本阶段尚未完成：当前后台执行仍依赖服务进程内 dispatcher，不是独立持久队列；取消、
受控重试、结果制品拆分、分页鉴权和 UI 均仍为后续工作。服务重启恢复与租约/心跳的
最小基础已在下一节补充，但不代表生产任务队列已经完成。涨跌停、公司行为、容量和
完整逐日权益等交易语义也没有因 API 接入而改变完成状态。

#### M4 任务租约与重启恢复基础补充记录

持久化 API 第一阶段之后，继续补齐了不会因普通进程重启永久卡死的最小恢复路径：

- 迁移 `0039_multi_asset_run_leases.sql` 增加 `worker_token`、`lease_expires_at`、
  `attempt_count` 和状态/租约联合索引，并已在当前开发数据库成功应用。
- Worker 通过数据库条件更新原子抢占 `queued` 任务；领取时生成不可复用的 worker
  token。进度、心跳、完成和失败写入必须同时匹配 `running + worker_token`，因此旧
  Worker 即使在租约转移后继续运行，也不能覆盖新 Worker 的状态或结果。
- 运行中每 30 秒续租，租约窗口为 10 分钟。服务成功监听端口后，才会把无租约或租约
  已过期的 `running` 任务重新放回 `queued`，并恢复尚未处理的持久化任务，避免新旧
  服务在端口切换期间同时接管后台工作。
- API 新任务和启动恢复任务统一进入进程内 dispatcher；当前并发上限固定为 2，同一
  运行 ID 在排队或执行期间不会重复入队，并暴露 active/queued/concurrency 内部统计。
  单元测试覆盖并发上限、去重、失败后继续排空；它解决单实例内无上限并发，不替代
  后续的独立持久队列。
- `npm run multi-asset:lease-smoke` 使用真实 MySQL 并发执行两个抢占请求，确认只有一个
  Worker 获胜；随后强制租约过期并重新领取，确认旧 token 写入被拒绝、新 token 可以
  继续推进。启动恢复也已在线验证：测试运行
  `761f5bfe-46ad-4a25-8eed-fe82441053cb` 在重启后由新 Worker 接管，`attempt_count` 从
  2 增至 3 并最终进入 `completed`。
- 邮件模板历史问题已同期收口：模板装饰图和亮点图标不再使用 data-URI SVG 或
  `<img>`，用户 Markdown 的远程图片、原始 HTML 和危险链接继续被过滤；安全链接仍
  可保留。全量后端 99 个测试文件、434 条测试及 TypeScript 构建均通过。
- 重启后的在线 API 又通过一轮真实 dispatcher 运行：运行
  `6ae44875-1f23-43e3-b482-04b89a2f0ebf` 首次领取（`attempt_count=1`）后进入
  `completed`，结果 hash 与既有黄金运行一致；健康检查显示并发上限为 2，验证结束时
  active/queued 均为 0。

上述实现仍是“数据库状态机 + 有界单服务实例内调度”的恢复基础，不是生产级分布式
任务系统。以下项目继续保持未完成：独立 Worker/持久队列及跨实例背压、取消令牌、按错误
分类的受控重试、最大尝试次数与死信处理、任务/结果制品拆分、进度事件推送和跨主机
故障注入。UI 基础流程在下一节记录，但不改变上述生产化能力的未完成状态；M4 整体
仍不得标记为完成。

#### M4 UI 优先实施补充记录

在不扩大后端完成口径的前提下，已增加独立的“多资产研究”工作区，并接入现有侧边栏、
明暗主题和 M4 计划/运行 API：

- 页面可创建并冻结沪深 300 研究计划，配置研究区间、周/月调仓、Top N、等权/评分
  加权、总仓位、单标的和最低现金约束；提交前执行 `Top N × 单标的上限` 的可承载
  仓位校验。当前 UI 只开放已经具备真实快照基础链路的沪深 300，不提前展示中证 500
  或全 A 股为可执行能力。
- 计划列表展示绑定快照和主要配置；运行列表展示排队、运行、完成、失败状态及阶段进度。
  存在活动任务时每 1.5 秒轮询，页面关闭不影响服务端任务继续执行。
- 完成态展示初始资金、期末权益、累计收益率、交易成本、订单/调仓数量、权益曲线、
  资金账本和交易清单；失败态原样展示服务端错误码与错误信息，不在前端伪造自动修复。
- 1280px 和更窄视口采用计划/运行并排、结果全宽或单列布局，消除内部横向溢出；明暗
  主题均完成真实浏览器检查，交互行保持稳定背景与可见键盘焦点。
- Python 计划 Worker 的默认脚本路径改为优先按模块位置解析，并为 Vitest 的非文件 URL
  转译环境提供显式仓库/服务目录候选，避免测试工作目录变化导致黄金样例误失败。

本节只表示“计划创建 → 启动运行 → 轮询进度 → 审阅结果”的 UI 基础流程已接入。
取消、用户触发重试、死信管理、SSE/WebSocket 进度推送、结果制品下载、分页和权限控制
仍未实现；独立 Worker/持久队列、跨实例背压和完整交易语义也继续保持未完成，因此 M4
整体状态仍为实施中。

#### M4 v1 完成验收记录（2026-08-01）

本节覆盖并取代前述各阶段记录中的“仍未完成”状态描述；前述文字保留用于说明实施
顺序。M4 v1 的冻结范围是：沪深 300 时点成分股、已发布 `momentum_20` 因子、周/月
调仓、等权/评分权重、A 股只做多组合撮合。该冻结范围已经完成，未将中证 500、全 A
股任意过滤、多因子优化或任意 Python 纳入本里程碑。

完成项与验收证据：

- `0040_multi_asset_production_queue.sql` 将运行状态扩展为持久队列，支持原子领取、租约、
  心跳、最多三次指数退避、死信、排队态/运行态取消、人工重试和完整事件审计。生产可设置
  `MULTI_ASSET_EMBEDDED_WORKER=false`，由独立 `npm run multi-asset:worker` 进程消费；
  API 进程不再在缺少调度器时偷偷执行任务。
- 进度事件提供增量查询和 SSE；调仓计划与执行结果使用原子文件写入，并在数据库保存
  SHA-256、字节数、媒体类型和下载清单。UI 已接入取消、重试、实时刷新、制品下载和
  因子/策略治理绑定。
- TypeScript 权威撮合已从仅调仓日升级为逐日盯市，处理整手、先卖后买、佣金、印花税、
  滑点、停牌、主板/创业板/科创板/北交所及 ST 涨跌停、成交量 10% 容量、部分成交、
  复权参数变化对应的送转数量和现金分红、现金非负、逐日权益及换手。组合账本仍是单一
  资金池，不是单股票结果事后拼接。
- Python Worker 使用 `-I` 隔离模式、最小环境变量、独占临时目录和文件交换；Node 限制
  输入、输出和墙钟时间并保证清理，POSIX 额外限制地址空间、CPU 与文件描述符。计算
  平面仍只能返回严格 `RebalancePlan`，不能写现金、持仓、订单或最终权益。
- 多资产信号 SQL 直接复用因子研究的 `compileBuiltinFactorSql(momentum_20)`，冻结时只
  接受数据库中已发布、方向与表达式一致的版本。绑定冠军/挑战者时要求策略处于
  `validated/paper/champion`、快照一致且确实包含该因子版本。
- `npm run multi-asset:production-smoke` 在真实 MySQL 上通过：排队取消、运行取消、自动
  重试、死信、人工重试、独立事件和双制品均符合预期；完成运行
  `9486bc3b-816b-4ab4-9ecb-b3779eaec6c9` 生成 39 个逐日账本点及两个可校验制品。
- `npm run multi-asset:snapshot-smoke` 在快照
  `eb260586-a8d0-4c24-9171-218b1222341c-20260731103228` 上再次通过：12,900 条时点
  输入、9 次调仓、Python/DuckDB 决策完全一致、106 笔成交、39 个逐日账本点，期末
  权益 1,038,604.49 元。该数值只用于链路复现，不代表收益承诺。
- 全量 155 个测试文件、882 项测试通过；前端生产构建、服务端 TypeScript 类型检查、
  0040 迁移、在线健康检查及明暗主题浏览器检查均通过。

M4 v1 之后已于 2026-08-01 完成中证 500/全 A 股股票池、通用多因子优化器、公告日
基本面和行业中性扩展。Linux 容器级任意代码沙箱、分布式消息中间件和多租户鉴权仍属于
后续里程碑，不影响本节 M4 v1 的历史冻结结论。

M4 扩展的实施顺序、Schema 演进、时点数据口径和逐项验收条件见
[M4 多资产研究扩展任务清单](./M4_EXTENSION_BACKLOG.md)。E1–E7 已单独通过阶段验收，
不追溯改写本节 M4 v1 的历史验收证据。
