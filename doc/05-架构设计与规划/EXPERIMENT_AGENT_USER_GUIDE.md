...
## 5. 使用多资产研究 Agent（M4 v1）

### 5.1 创建冻结计划

1. 从左侧进入“多资产研究”；
2. 点击“新建研究计划”；
3. 设置名称、回测区间、周/月调仓、Top N、等权/评分加权、仓位上限和最低现金；
4. 可选填写已发布因子版本 ID 与冠军/挑战者策略版本 ID；
5. 点击“冻结计划”。

当前资产域固定为沪深 300。治理绑定只接受已发布 `momentum_20`，策略版本必须处于
`validated`、`paper` 或 `champion`，并且绑定相同快照和因子版本。
...
| 因子治理绑定失败 | 确认版本已发布且为 `momentum_20`，快照和策略状态一致 |
