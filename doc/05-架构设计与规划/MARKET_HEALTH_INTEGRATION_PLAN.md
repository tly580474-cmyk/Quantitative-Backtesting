...
> 文档状态：第一阶段已上线验证，监控与季度 reconciliation 待继续完善
> 制定日期：2026-08-29  
...
### 10.0 2026-08-29 实施记录

- [x] 阶段 A：新增宏观观察值、健康度快照表、仓储、只读 API 与 freshness 契约。
- [x] 阶段 C（首个纵向切片）：完成 AKShare PPI 传输层、版本去重、NEC 固定权重计算、自然日调度与启动补偿。
- [x] 阶段 E（第一版）：完成五轴下拉、用户选择持久化、30 分钟低频缓存、独立刻度与方向文案。
- [x] 阶段 B：VPI、FHI 生产计算、质量门禁、自然日物化与手工 dry-run/apply 入口。
- [x] 阶段 D：MSH 冻结公式、行情/指数/行业完整性门禁与日终物化。
- [ ] 阶段 F：管理台监控、季度 reconciliation、视觉矩阵和灰度发布。

#### 2026-08-29 上线验证

- 已执行 `0044_market_health.sql`，并首次发布 MSH、FHI、NEC、VPI 四类快照；重复执行不会重复发布未变化的数据。
- `market-health:materialize -- --apply` 现会完整执行 NEC、FHI/VPI 与 MSH，适合作为手工补偿入口。
- 服务内调度已启用：10:30 自然日检查 PPI/NEC，20:30 读取最新研究快照并物化 FHI、VPI、MSH；启动时会立即检查当日漏跑任务。
- Windows 任务 `QuantBacktest-ResearchSnapshot` 已恢复为 Ready，按 09:00、17:00、18:30 三个时点尝试执行；脚本先判断中国交易日，再依次更新指数、指数成分、申万行业、分红并原子发布研究快照。
- 已手工补跑申万行业链路，行业行情由 2026-07-30 推进至 2026-08-28；新快照共 17,258,591 行并通过校验。
- 最终只读 API 返回四轴均为 `current`：MSH 45.99、FHI 47.18、NEC 66.28、VPI 65.73；前端下拉已逐项验证，MSH 展示为“结构中性”、数据期 2026-08-28。
- 调度实跑记录已写入 `market_data_collector_runs`：`market_health_nec:2026-08-29` 与 `market_health_daily:2026-08-29` 均成功，证明启动补偿和幂等跳过可工作。
- 健康轴不再共用单一温度计：MSH 使用结构雷达图，FHI 使用分期柱状图，NEC 使用带零轴的正负宏观柱状图，VPI 使用五档压力区间折线图；MSI 继续保留原温度计语义。
- 只读 overview 同时返回同模型最多 120 个已发布/历史展示点。生产历史不足时页面明确提示“历史快照仍在积累”，不把实验产物伪装成线上历史；新快照会自动扩展图形序列。
- 已增加 `market-health:backfill` 历史重算入口，直接读取当前验证通过的研究快照并沿用生产冻结公式，不依赖实验 CSV。FHI 按财报公告日做 ASOF 匹配并选取每个报告期首次达到正式覆盖率的快照；NEC 按 PPI 观察月和可用日逐月计算；VPI 按交易日计算后保存月末档案。
- 2026-08-29 首次回填结果：FHI 81 点（2005Q1～2026Q2）、NEC 207 点（2009-04～2026-06）、VPI 301 点（2001-07～2026-07）。历史行写为 `superseded`，不会取代最新 `published`；API 将日频历史压缩为月末点并最多返回近 120 期。

当前实现明确把 AKShare 的东方财富抓取地址记录为 `retrievalUrl`，把统计局记录为
`authorityKey=nbs`；这不等同于统计局官方直连接口。后续官方抽样核对完成前，页面只展示
数据期与模型状态，不宣传为“官方直连”。

阶段 B 使用冻结实验权重：FHI 为盈利能力 50% 与盈利增长 50%，VPI 为 PE 压力 50% 与
PB 压力 50%。分数仍使用当前观察之前最多 756 个交易日、至少 252 个观测标准化。近 900
个交易日实测正 PE 覆盖率约为 71%～79%，因此 VPI 正式发布门槛设为 70%、最低展示门槛
设为 50%；这是数据完整性门禁，不参与分数计算。FHI 正式/最低门槛保持 85%/30%。

手工演练默认只读，不写数据库：

```powershell
cd server
npm run market-health:materialize
npm run market-health:backfill
```

完成迁移并确认快照后，可显式添加 `-- --apply` 发布 NEC/FHI/VPI/MSH。自动任务默认 20:30 按自然日
检查研究快照；FHI 只在财务数据集指纹变化时发布，VPI 只在行情日期推进时发布。

MSH 继续使用 v1 冻结权重：趋势 30%、参与度 25%、下行风险 30%、流动性 15%。日终
任务读取复权行情、五个核心指数和申万行业行情，取三类数据共同可用的最近日期；任何一类
未推进都不会冒充最新日期。2026-08-29 的真实快照演练中，个股行情截至 08-27，但申万行业
行情截至 07-30，因此 MSH 只能发布 07-30 快照并被 freshness 标记为 stale。这是预期降级，
该降级验证后已恢复行业行情计划任务并完成补跑，MSH 随新快照推进至 08-28；以后仍坚持由
三类数据共同日期决定 as-of，不用中性值或较新的个股日期覆盖滞后的行业数据。

历史回填同样默认 dry-run；确认输出的起止日期和点数后，使用
`npm run market-health:backfill -- --apply` 幂等写入历史档案。模型公式发生变化时必须升级
`model_version` 后重新回填，不能在原版本下覆盖历史分数。
...
- 重构温度计为通用卡片。
- 接入下拉选择、按轴适配的雷达/柱状/正负柱状/区间折线、不同刻度、频率和 freshness 标签。
- 缓存用户选择，适配移动端和深色模式。
