...
研究快照目录的默认值定义在 `server/src/config.ts` 的 Zod schema 中：

```text
RESEARCH_SNAPSHOT_ROOT=./data/research-snapshots
```

该变量不要求实际出现在 `.env` 中。配置加载顺序为：

1. 命令行 `--snapshot-root`；
2. 进程环境变量 `RESEARCH_SNAPSHOT_ROOT`；
3. `src/config.ts` 中的默认值 `./data/research-snapshots`。

从 `server` 目录执行 CLI 时，默认路径对应：

```text
D:/github_public_repo/量化回测/server/data/research-snapshots
```
...
| `sw_industry_definitions` | SW2021 一级、二级、三级行业定义 | `taxonomyStart` |
| `sw_industry_memberships` | 股票申万行业归属及有效区间 | `effectiveFrom`、`effectiveTo` |
| `sw_industry_bars` | 31 个申万一级行业历史行情 | `tradeDate` |
...
分钟数据不属于研究快照，也不会自动注册为视图。`duckdbCli.ts` 不读取或展开 `MINUTE_DATA_ROOT`；分钟查询必须在 SQL 的 `read_parquet(...)` 中显式提供文件路径或通配符。

`src/config.ts` 虽然为其他分钟数据导入器和应用接口定义了 `MINUTE_DATA_ROOT` 默认值，但该配置不参与 DuckDB CLI 的分钟路径解析。修改 `.env` 中的 `MINUTE_DATA_ROOT` 不会自动改变本指南中的 CLI 查询路径。
...
```powershell
npm run duckdb -- query --sql "SELECT symbol, name, level1Code, level1Name, level2Code, level2Name, level3Code, level3Name, effectiveFrom, effectiveTo FROM sw_industry_memberships WHERE symbol='002155' AND effectiveTo IS NULL"
```

查询某个历史日期的申万行业归属：

```powershell
npm run duckdb -- query --sql "SELECT symbol, level1Name, level2Name, level3Name FROM sw_industry_memberships WHERE symbol='002155' AND TIMESTAMP '2024-09-25 23:59:59' >= effectiveFrom AND (effectiveTo IS NULL OR TIMESTAMP '2024-09-25 23:59:59' <= effectiveTo)"
```

同时查询股票所属一级行业的涨跌幅：

```powershell
npm run duckdb -- query --sql "SELECT membership.symbol, membership.name, membership.level1Name, bar.tradeDate, bar.close AS industryClose, bar.changePercent AS industryChangePercent FROM sw_industry_memberships AS membership INNER JOIN sw_industry_bars AS bar ON bar.taxonomyKey=membership.taxonomyKey AND bar.indexCode=membership.level1IndexCode WHERE membership.symbol='002155' AND bar.tradeDate='2026-07-15' AND CAST(bar.tradeDate AS TIMESTAMP) >= membership.effectiveFrom AND (membership.effectiveTo IS NULL OR CAST(bar.tradeDate AS TIMESTAMP) <= membership.effectiveTo)"
```

当前分类体系固定为 `SW2021`。申万行业不等同于概念题材；东财概念板块尚未接入，也不会混入申万行业视图。

## 8. 分钟行情查询示例

以下示例直接使用当前机器上的实际绝对路径。若数据目录不同，必须在 SQL 中替换路径。
...
CLI 都会重新读取按“`--snapshot-root` → 环境变量 → `src/config.ts` 默认值”解析出的研究快照目录中的当前指针。因此新的研究快照完成发布后，无需修改 DuckDB 文件或重建 CLI 视图，下一次命令会自动查询同一版本中的股票日线和参考数据。
